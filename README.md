...
click breaks · right click places (or `J` / `L`) · `Esc` quits.
...
columns around the player live in an `Array<U32>`, each column one word
whose bit `y` says "there is a block at height `y`". Every ray reads it
through its handle; an edit is one `Array.set` on the host; a built world
costs what an untouched one does. Thirty-two heights in one word is what
makes it cheap: a column is a machine word, and break or place is one bit.

**The world is endless.** The terrain is seeded value noise, three octaves,
a pure function of `(x, z)`. The array holds a ring of 128×128 columns
around the player, and a `Map` holds the columns the player edited. World
column `(x, z)` sits at slot `x·128 + z`; the array reads its index modulo
its size, so any 128×128 window falls one-to-one on the 16384 slots, and a
reader carries one word, `base = ox·128 + oz`, to find local column
`(lx, lz)` at `base + lx·128 + lz`. The render and the physics only see
local coordinates in `[0, 128)`. The corner follows the player one column at
a time, loading the row that came into view, from the map if it was edited
and from the noise if not; an edit is a bit in the ring and the whole
column in the map, so it is there when you come back.
...
the ring spans its 16384 slots exactly; placing sets one bit, breaking
clears it, breaking air changes nothing; no key touches the mouse's bits of
the held mask. `bend PROOF.bend` is the gate. The float physics, a player
